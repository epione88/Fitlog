...
// Core tone player
function playTone(freq, type, duration, vol, startTime, ac) {
...
const SOUND_THEMES = {
  classic: {
    label: "Classic Beeps",
    workStart:     (ac, vol, t) => { playTone(880, "square", 0.12, vol, t, ac); playTone(1100, "square", 0.12, vol, t+0.15, ac); },
    restStart:     (ac, vol, t) => { playTone(440, "sine",   0.25, vol, t, ac); },
    countdown:     (ac, vol, t) => { playTone(660, "square", 0.08, vol, t, ac); },
    countdownFinal:(ac, vol, t) => { playTone(990, "square", 0.15, vol, t, ac); },
    complete:      (ac, vol, t) => {
      [523, 659, 784, 1047].forEach((f, i) => playTone(f, "sine", 0.3, vol, t + i*0.12, ac));
    },
  },
  gym: {
    label: "Gym Whistle",
    workStart:     (ac, vol, t) => {
      // short sharp blast
      for (let i = 0; i < 2; i++) { playTone(1200, "sawtooth", 0.08, vol * 0.7, t + i*0.12, ac); }
    },
    restStart:     (ac, vol, t) => { playTone(600, "sine", 0.3, vol, t, ac); },
    countdown:     (ac, vol, t) => { playTone(800, "sine", 0.07, vol, t, ac); },
    countdownFinal:(ac, vol, t) => { playTone(1200, "sine", 0.12, vol, t, ac); },
    complete:      (ac, vol, t) => {
      for (let i = 0; i < 3; i++) { playTone(1200, "sawtooth", 0.1, vol*0.8, t + i*0.18, ac); }
    },
  },
  minimal: {
    label: "Minimal Clicks",
    workStart:     (ac, vol, t) => { playTone(1000, "sine", 0.04, vol, t, ac); playTone(1500, "sine", 0.04, vol, t+0.06, ac); },
    restStart:     (ac, vol, t) => { playTone(500,  "sine", 0.04, vol, t, ac); },
    countdown:     (ac, vol, t) => { playTone(800,  "sine", 0.03, vol, t, ac); },
    countdownFinal:(ac, vol, t) => { playTone(1200, "sine", 0.05, vol, t, ac); },
    complete:      (ac, vol, t) => { [400,600,800,1000,1200].forEach((f,i) => playTone(f,"sine",0.08,vol,t+i*0.08,ac)); },
  },
};

// Main useSound hook — call play("workStart") etc.
function useSound(settings) {
  const play = useCallback((event) => {
    ...
    try {
      ...
      const theme = SOUND_THEMES[settings.soundTheme] || SOUND_THEMES.classic;
      const fn    = theme[event];
      if (fn) fn(ac, vol, ac.currentTime);
...
const DEFAULT_SETTINGS = {
  ...
  hapticsEnabled: true,
  soundTheme:     "classic",
  countdownBeeps: true,    // beep at 3,2,1 before phase ends
};
    ...
    <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.75)", zIndex: 100, display: "flex", flexDirection: "column", justifyContent: "flex-end" }}
      ...
      <div style={{ background: "#1a1218", borderRadius: "20px 20px 0 0", padding: "20px 20px 48px", maxHeight: "85vh", overflowY: "auto" }}>
        ...
        {settings.soundEnabled && <>
          ...
          <Row label="Sound Theme" sub="Style of timer cues">
            <div style={{ display: "flex", flexDirection: "column", gap: 6, alignItems: "flex-end" }}>
              {Object.entries(SOUND_THEMES).map(([key, theme]) => (
                <button key={key}
                  onClick={() => { set("soundTheme", key); setTimeout(() => play("workStart"), 50); }}
                  style={{
                    ...chipStyle, padding: "5px 12px", fontSize: 12,
                    background: settings.soundTheme === key ? "#ef6c35" : "rgba(255,255,255,0.08)",
                    color: settings.soundTheme === key ? "#fff" : "rgba(255,255,255,0.5)",
                    border: "none",
                  }}>
                  {theme.label}
                </button>
